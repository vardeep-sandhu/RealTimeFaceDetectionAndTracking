...
def main():
    ...
    mean_embds = get_mean_embds("path")
    #init detector
    ...
    labels2int = {cls:float(label) for label, cls in classes_list}
    ...
    while True:
        ...
        interested_track = trackers[trackers[:, -1] == labels2int[person_track]]
        if len(interested_track) == 1:
            d = interested_track[0]
            ...
            bb_x = int(((d[0] + d[2]) / 2) * scaling)
            print(bb_x)
            
        if len(interested_track) > 1:
            print("Multiple Detections here, needs to be fixed")
            # Most confidence 
            # Most close in distance
            pass
